...
export async function requestDefaultSearch(): Promise<DefaultSearch> {
	try {
		const res = await service.get('/api/search/default');
		if (res.data) {
...
export async function requestHotSearch(): Promise<HotSearchItem[]> {
	try {
		const res = await service.get('/api/search/hot/detail');
		if (res.data) {
			return res.data;
		}
		return Promise.reject();
...
export async function requestSearchSuggest(keywords: string): Promise<SearchSuggest> {
	try {
		const res = await service.get('/api/search/suggest', { params: { keywords } });
		if (res.result) {
			return res.result;
		}
